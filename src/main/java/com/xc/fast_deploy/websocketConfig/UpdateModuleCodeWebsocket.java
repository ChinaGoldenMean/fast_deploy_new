 ...
 * 更新代码实时查看更新进度
 */
@ServerEndpoint(prefix = "netty.websocket", value = "/websocket/update/module/package")
@Component
@Slf4j
public class UpdateModuleCodeWebsocket {
...
//            }
  ...
  public void onMessage(Session session, String message) {
    ...
    if (StringUtils.isNotBlank(message) && StringUtils.isNotBlank(message.trim())) {
      ...
      ThreadPoolExecutor poolExecutor = new ThreadPoolExecutor(hashSet.size(), hashSet.size(), 0L, TimeUnit.SECONDS, new ArrayBlockingQueue<>(100), new ThreadPoolExecutor.CallerRunsPolicy());
