...
// 读取输出直到找到 echo 命令的输出行
  ...
  public static void cloneSubOrPull(String repositoryURL, String path, String username, String password, String gitBranch) {
    Git git = null;
    CredentialsProvider provider = new UsernamePasswordCredentialsProvider(username, password);
    try {
      git = Git.open(new File(path));
      //不用设置分支去做git pull 默认使用当前分支去做git pull
      git.pull().setCredentialsProvider(provider).setTimeout(30).call();
      //如果不存在sh文件，则需要重新clone
    } catch (Exception e) {
      // 目录打开失败，不是Git代码库
      autoGitClone(git, repositoryURL, path, provider, gitBranch);
    }
  ...
  public static void cloneOrPull(String repositoryURL, String path, String username, String password, String gitBranch) {
    ...
    try {
      ...
      File shFile = FileUtil.loopFiles(new File(path), 1, file -> file.isFile() && file.getName().endsWith(".sh")).stream().findFirst().orElse(null);
      if (shFile == null) {
        autoGitClone(git, repositoryURL, path, provider, gitBranch);
      }
  ...
  private static void autoGitClone(Git git, String repositoryURL, String path, CredentialsProvider provider, String gitBranch) {
    try {
      //先删除
      if (git != null) {
        git.close();
